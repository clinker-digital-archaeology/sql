...
SELECT 
    ...
    ) AS total_purchases_per_product
FROM customer_purchases
...
-- Note: SQLite may not always support REGEXP without extensions, so GLOB is used as a fallback.
-- REGEXP version:
SELECT *
FROM product
...
SELECT 
    v.vendor_id,
    p.product_name,
    COUNT(DISTINCT c.customer_id) * 5 * AVG(cp.cost_to_customer_per_qty) AS total_revenue
FROM vendor AS v
CROSS JOIN product AS p
CROSS JOIN customer AS c
JOIN customer_purchases AS cp
    ON cp.product_id = p.product_id
GROUP BY v.vendor_id, p.product_name;
...
-- Insert a new product record with updated timestamp*/
...
VALUES (
    ...
    'unit',                         
    CURRENT_TIMESTAMP  
);  -- <-- IMPORTANT: close the statement with a semicolon
